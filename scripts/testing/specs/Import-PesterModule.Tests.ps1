<#PPester tests for Import-PesterModule (scripts/testing/helpers/PesterHelpers.psm1)
...
Describe 'Import-PesterModule' {
    BeforeAll {
        ...
        try {
            $helperPath = Resolve-Path -Path $helperPath -ErrorAction Stop | Select-Object -ExpandProperty Path
        }
        catch {
    ...
    Context 'import by ModulePath' {
        It 'imports a local module file and logs success' {
            $tmpModule = Join-Path $TestDrive ([guid]::NewGuid().ToString() + '.psm1')
            # Create a minimal module file that exports a harmless function
            $moduleSource = @'
...
'@
            ...
            $logger = [KalmScriptLogger]::Start('pesterhelper', $logDir)

            $moduleInfo = Import-PesterModule -ModulePath $tmpModule -Logger $logger

            $moduleInfo | Should -Not -BeNullOrEmpty
            # log must contain an Imported module record
        ...
        It 'throws when ModulePath does not exist and logs failure' {
            ...
            $content | Should -Match 'Failed to import|Module import failed|ModulePath not found'
        }
    }
