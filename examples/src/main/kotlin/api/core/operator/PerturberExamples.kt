...
fun main() {
	...
	println("Lambda perturber         : ${lambdaPerturberExample(candidate, Random(seed))}")
	...
	println("Composed perturber        : ${composedPerturberExample(candidate, Random(seed))}")
}
...
fun composedPerturberExample(candidate: Solution<Double>, rng: Random): Solution<Double> {
	...
	val composed = Perturber<Double> { solution -> clamp.perturb(jitter.perturb(solution)) }
