...
/**
 * Produces an [io.kotest.property.Arb] that generates [Either]-wrapped [Size] values within [range].
 *
 ...
 * @return An [io.kotest.property.Arb] of `Either<SizeError, Size>` reflecting validation of each sampled value.
 */
internal fun Arb.Companion.size(range: IntRange = 0..1024): Arb<Either<SizeError, Size>> =
    Arb.int(range)
        .map { Size(it) }
 ...
 * @return An [io.kotest.property.Arb] of `Either<SizeError, Size>` within `[min, max]`.
 */
internal fun Arb.Companion.size(
    min: Int = 0,
    max: Int = 1024
...
/**
 * Creates an [io.kotest.property.Arb] that generates valid [Size] instances within the given [range].
 *
 ...
 *   Defaults to `0..1024`.
 * @return An [io.kotest.property.Arb] producing valid [Size] values within the specified range.
 * @throws IllegalArgumentException if a generated value is invalid for [Size].
 */
internal fun Arb.Companion.validSize(range: IntRange = 0..1024): Arb<Size> =
    Arb.int(range).map { Size.ofOrThrow(it) }

/**
 * Creates an [io.kotest.property.Arb] that generates valid [Size] instances within the inclusive range `[min, max]`.
 *
 ...
 * @return An [io.kotest.property.Arb] producing valid [Size] values between [min] and [max].
 * @throws IllegalArgumentException if a generated value is invalid for [Size].
 */
internal fun Arb.Companion.validSize(
    min: Int = 0,
    max: Int = 1024
